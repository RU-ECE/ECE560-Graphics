...
class camera {
  public:
    ...
    uint32_t compute_color(const color& pixel_color) const {
...
//            std::clog << "\rScanlines remaining: " << (image_height - j) << ' ' << std::flush;
            for (int i = 0; i < image_width; i++) {
                ...
                pixels[j * image_width + i] = compute_color(pixel_samples_scale * pixel_color);
//                write_color(std::cout, pixel_samples_scale * pixel_color);
        ...
        for (int j = 0; j < image_height; j++) {
            for (int i = 0; i < image_width; i++) {
                int r = (pixels[j * image_width + i] >> 16) & 0xFF;
                int g = (pixels[j * image_width + i] >> 8) & 0xFF;
                int b = pixels[j * image_width + i] & 0xFF;
                std::cout << r << ' ' << g << ' ' << b << '\n';
            }
        }
        std::clog << "\rDone.                 \n";
